...
function Build-ReviewPackMarkdown {
    ...
    [void]$builder.AppendLine("| --- | --- | --- |")
    [void]$builder.AppendLine("| Draw Choice | Three comparable cards with shape, ingredient, value/risk, ``Fit``, ``Heat``, ``Role``, and distinct ``SAFE``/``GREEDY``/``UTILITY``-style intent visible within 3 seconds. | ``FIX_LAYOUT`` if clipped or overlapping; ``FIX_FEEDBACK`` if the choice tradeoff is unclear or the cards read as the same intention. |")
    ...
    [void]$builder.AppendLine("| --- | --- | --- |")
    [void]$builder.AppendLine("| Read | Can the player compare the current offer and understand the safest/greediest/utility option within 3 seconds? | Draw cards do not explain fit, Heat, role, recipe progress, or the next decision pressure, or all three cards feel like the same intent. |")
    ...
    [void]$builder.AppendLine("| Pressure | Does combat visibly rise from Build to Peak, and can the player tell source -> target -> result? | The battlefield lacks readable lane pressure, attack trails, action labels, Heat pressure, enemy approach, or cause/effect labels such as ``HIT>Z``, ``LEAK``, and ``BITE>TRK``. |")
