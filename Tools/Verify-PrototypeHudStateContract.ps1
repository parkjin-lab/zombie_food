...
Add-ContractCheck $checks "combat_feedback" "floating_damage_text_explains_hits_and_leaks" ($sources.hud + $sources.enemyVisuals) @(
    ...
    'SpawnEnemyHitSlash(laneRoot, source.Rect.anchoredPosition, 0f, 1.08f * punch',
    'SpawnEnemyHitSlash(laneRoot, source.Rect.anchoredPosition, 52f, 0.80f * punch',
    'SpawnEnemyHitSlash(laneRoot, source.Rect.anchoredPosition, -48f, 0.66f * punch',
    'SpawnEnemyHitImpactCore(laneRoot, source.Rect.anchoredPosition, punch);',
    'SpawnEnemyAttackTrailSegment(laneRoot, startX, y, width, laneHeight, angle',
...
Add-ContractCheck $checks "editor_helpers" "review_pack_collects_evidence_for_visual_decision" $sources.playModeReviewPackWriter @(
    ...
    '| Read | Can the player compare the current offer and understand the safest/greediest/utility option within 3 seconds?',
    '| Commit | Does placement resolve quickly into a clear next state?',
    '| Pressure | Does combat visibly rise from Build to Peak, and can the player tell source -> target -> result?',
    '| Payoff | Does the result explain why the last choice mattered?',
