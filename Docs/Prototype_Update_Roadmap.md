...
- Wave Combat showcase readability update: verification capture now keeps thicker attack trails and high-contrast `HIT>Z -12`, `HIT>Z KO`, `LEAK`, and `BITE>TRK -7` markers alive longer so the next retake can judge combat cause/effect with less manual timing pressure.
...
- Session resource visibility update: session status and Play Mode evidence preflight now surface `asset_planned_missing`, currently `14`, so the planned VFX/SFX backlog is visible at session start without being treated as a runtime blocker.
- Review pack resource visibility update: `Write-PrototypePlayModeReviewPack.ps1` now includes a `Planned Resource Backlog` section and JSON fields for `asset_status` / `asset_planned_missing`, keeping visual review and resource backlog triage in the same evidence artifact.
...
- Cadence overlap telemetry update: `WaveCadencePlan` now exposes `LastWaveCadenceSpikeOverlapCount`, and UX telemetry exports `spike_overlap_count` so planned spikes can be tuned by overlap intensity instead of only by label.
- Read choice diversity note: sub-agent review on 2026-06-14 found this is a good small candidate, but it should wait until the current review pack PASS/FIX/BLOCKED decision is recorded so Draw Choice evidence does not become stale.
...
3. `Rhythm Beat HUD/Telemetry`: expose `Read`, `Commit`, `Pressure`, `Payoff`,
   ...
   line reuse the same label. Current beat duration and transition count are now
...
4. `Pressure Ramp Tuning`: make the 20-second wave visibly rise from readably
   ...
   Source status: second pass complete as bounded spawn tuning. The model now
   labels pressure as `Build`, `Climb`, or `Peak`, exposes a smooth intensity
   value, applies `pressure_ramp_spawn_mult` from `x0.72` to `x1.28` to combat
   spawning, and exports the multiplier through HUD/UX telemetry. Remaining work
   is visual review and deciding whether Heat should also follow this curve.
...
guard the beat label, current duration, transition count, and CSV export columns.
