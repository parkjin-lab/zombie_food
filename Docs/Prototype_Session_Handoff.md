...
- Sub-agent review agrees the next fun work should not be a broad new mechanic. After Composer, Payoff-to-Read, Rhythm Beat, and beat-duration telemetry source guards, the next autonomous candidates are pressure ramp tuning and rest-phase reward, unless visual review says the current hints need tuning first.
...
27. After Play Mode evidence is PASS or has a concrete FIX record, verify whether the source-level `Wave Cadence Composer`, `Payoff-to-Read panel`, and `Rhythm Beat HUD/telemetry` make wave 3/4/5/7 beats, next-choice hints, current beat labels, duration, and transition count readable enough, then continue with pressure ramp tuning and rest-phase reward in that order.
