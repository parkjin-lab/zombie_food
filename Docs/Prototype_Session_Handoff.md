...
- `Tools/Show-PrototypeSessionStatus.ps1`: `prototype_session_readiness=needs_manual_playmode`, `gate_status=ok`, `asset_status=ok`, `layout_status=ok`, `playmode_record_status=not_recorded`, `static_status=ok`.
...
- `Tools/Gate-Verification.ps1`: includes asset and layout verification in JSON gate output.
