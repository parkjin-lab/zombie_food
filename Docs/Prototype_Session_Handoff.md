...
- Play Mode suite evidence is now `captured_manual` with all four required
  states represented by manual screenshot registration.
- Screenshot evidence is `suite_ready` with 7/7 valid portrait PNGs and complete
  required-state coverage.
- Review pack preview is `ready_for_visual_review`.
...
- Play Mode suite evidence currently reports `playmode_suite_status=captured_manual`, `captured_count=4/4`, `suite_capture_source=manual screenshot registration`, and complete labeled coverage for Draw Choice, Pending Placement, Invalid Placement, and Wave Combat.
- Existing Play Mode screenshots currently report `playmode_screenshot_status=suite_ready`: seven portrait PNGs pass machine quality, four required states are covered through the suite manifest, one PNG is triaged as a non-required Build Flow idle screenshot, `manual_registration_candidate_count=0`, and `triaged_non_state_count=1`.
...
- `Payoff-to-Read panel` second source pass is complete: `Next` hints now include a cause prefix, for example `Leak x2 -> Stabilize lanes`, `PeakHeat +19 -> Pick COOL/SAFE`, and `3 KO -> Push damage`.
- `Commit -> Pressure` starter cue source pass is complete: after a wave-choice block is placed and combat unlocks, the HUD surfaces `Wave live: <ramp> x<mult>. Watch lanes.` so placement resolves into a visible wave-start beat.
- `Review pack rhythm checklist` source pass is complete: `Tools/Write-PrototypePlayModeReviewPack.ps1` now asks reviewers to classify screenshot failures as `Read`, `Commit`, `Pressure`, `Payoff`, or `Release` before requesting broad mechanic or balance work.
- `Cadence overlap telemetry` source pass is complete: `WaveCadencePlan` now exposes `LastWaveCadenceSpikeOverlapCount`, HUD telemetry exports `spike_overlap_count`, and EditMode coverage locks Wave 3/4/5/7/15 overlap values so planned spikes can be tuned by intensity.
- `Rhythm Beat HUD/telemetry` first source pass is complete: the model resolves `Read`, `Commit`, `Pressure`, `Payoff`, and `Release`, the HUD status line shows the current beat, and UX telemetry panel/mini line/CSV export reuse the same beat label, current beat duration, transition count, and cadence summary.
- `Pressure Ramp Profile/Tuning` source pass is complete: the model labels wave pressure as `Build`, `Climb`, or `Peak`, exposes a smooth intensity value, applies a bounded combat spawn multiplier from `x0.72` to `x1.28`, and surfaces the profile through HUD/UX telemetry/CSV.
- `Rest-Phase Reward` first source pass is complete: boss/rest release windows now grant a small automatic `Repair`, `Cooling`, or `Stock` reward from the completed wave's leaks/HP/Heat/KOs/combo signal and surface it through HUD/UX telemetry/CSV.
- `Combat cause labels` first source pass is complete: truck HP loss floaters now distinguish `BITE`, `PRESSURE`, and `OVERHEAT` so Wave Combat review can judge why the truck is dying.
- `Combat flow labels` source pass is complete: battlefield floaters now use compact source-to-target result copy such as `HIT>Z -5`, `HIT>Z KO`, and `BITE>TRK -7` so combat feedback reads as a cause chain.
- `Payoff best contributor` first source pass is complete: wave outcome summaries now append a compact `Best ...` cause so review can judge whether the Payoff beat teaches what worked.
- `Draw card intent labels` first source pass is complete: draw cards now append a live-context `Intent` label (`SAFE`, `GREEDY`, `SYNERGY`, `UTILITY`, or `HOLD`) so the Draw beat can communicate why a card is stable, risky, synergy-building, flexible, or currently unplaceable.
- `Draw recipe progress preview` first source pass is complete: draw cards now append a compact `Recipe` line that marks seed, near-bingo, bingo, or double-bingo opportunities from current ingredient and shape counts.
- `Board-local invalid placement labels` first source pass is complete: blocked grid cells now show short local labels (`S2 OCCUPIED`, `S4 BOUNDS`, `S1 ANCHOR`) while the pending row keeps the longer recovery hint.
- `Rest reward visibility` first source pass is complete: rest phase now adds a compact `Release ...` reward chip to the mid-panel chip row, e.g. `Release Cooling | -16 Heat`.
- `Needed resource list` is updated: the asset verifier now reports 14 non-blocking `planned_missing` VFX/SFX items for attack source trails, impact/leak/payoff/release pulses, and rhythm audio under `Assets/Resources/FoodTruckPrototype`.
- `Resource backlog visibility` is updated: `Tools/Show-PrototypeSessionStatus.ps1` and `Tools/Invoke-PrototypePlayModeEvidencePreflight.ps1` now expose `asset_planned_missing=14` so planned VFX/SFX needs stay visible without blocking the prototype gate.
- `Review pack resource visibility` is updated: `Tools/Write-PrototypePlayModeReviewPack.ps1` now includes a `Planned Resource Backlog` section and JSON `asset_status` / `asset_planned_missing` fields for visual review context.
- Sub-agent review agrees the next fun work should not be a broad new mechanic. After Composer, Payoff-to-Read, Rhythm Beat, beat-duration telemetry, Pressure Ramp tuning, and Rest-Phase Reward source guards, the next autonomous candidates are rest reward display/value tuning and Heat ramp tuning, unless visual review says the current hints need tuning first.
...
27. After Play Mode evidence is PASS or has a concrete FIX record, verify whether the source-level `Wave Cadence Composer`, `Payoff-to-Read panel`, `Rhythm Beat HUD/telemetry`, and `Pressure Ramp Profile` make wave 3/4/5/7 beats, next-choice hints, current beat labels, duration, transition count, and Build/Climb/Peak ramp readable enough, then continue with pressure ramp value tuning and rest-phase reward in that order.
...
- Play Mode suite evidence is tracked and currently complete as manual evidence: playmode_suite_status=captured_manual, suite_capture_source=manual screenshot registration, captured_count=4/4, wave_combat_action_showcase_ready=false, wave_combat_action_showcase_reason=legacy_wave_combat_capture_without_attack_trails_and_labels. This status is visible from `Tools\Show-PrototypeSessionStatus.ps1` without opening the review pack.
- Play Mode screenshot evidence is tracked and currently suite-ready: 7 valid portrait PNGs, unlabeled_count=0, manual_registration_candidate_count=0, triaged_non_state_count=1, covered_state_count=4/4, and all required states are labeled through the suite manifest.
- Play Mode review pack writer works in preview mode and currently reports review_pack_status=ok, review_readiness=ready_for_visual_review, review_pack_visual_review_required=True, manual_registration_candidate_count=0, and triaged_non_state_count=1; it now includes suite evidence source, Wave Combat action showcase status, planned resource backlog, visual acceptance checklist, rhythm beat review checklist, and triaged non-state screenshots. Full generation writes the review pack.
...
- PlayMode evidence and review pack now report readiness for visual judgment: `review_readiness=ready_for_visual_review`, focused retake count is expected to be 0 unless evidence regresses, and commands_after_capture remain available only for future capture passes.
...
- Session status top issue is no longer missing state coverage; the next evidence action is to open the review pack and make intentional PASS/FIX/BLOCKED judgments while preserving the Wave Combat action-showcase guard.
...
Next priority:
Run Tools/Verify-PrototypePlayModeSuite.ps1, Tools/Verify-PrototypePlayModeScreenshots.ps1, Tools/Verify-PrototypePlayModeRecord.ps1, and Tools/Write-PrototypePlayModeReviewPack.ps1 -PreviewOnly -JsonOnly at session start. If they still report `captured_manual`, `suite_ready`, and `ready_for_visual_review`, do not create new focused-retake work. Open or regenerate the review pack, use the `Rhythm Beat Review Checklist` to name the weak beat (`Read`, `Commit`, `Pressure`, `Payoff`, or `Release`), and leave official PASS/FIX/BLOCKED recording to an intentional review step. Wave Combat still cannot be auto-PASSed while `wave_combat_action_showcase_ready=false`. After evidence is PASS or has a concrete FIX record, do not restart completed Composer/Payoff/Rhythm/Pressure/Rest source passes; first visually judge those changes, then choose rest reward display/value tuning, Heat ramp tuning, Read choice diversity, cadence overlap telemetry, or payoff hint visibility based on the recorded weak beat.
```
